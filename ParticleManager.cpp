...
void ParticleManager::LoadTexture() {
	...
	// WICテクスチャのロード
	result = LoadFromWICFile(L"Resources/pert.png", WIC_FLAGS_NONE, &metadata, scratchImg);
	assert(SUCCEEDED(result));
...
void ParticleManager::UpdateViewMatrix() {
	...
	//カメラの位置からワールド座標へのベクトル
	XMVECTOR tX = XMVector3Dot(cameraAxisX, reverseEyePosition);
	XMVECTOR tY = XMVector3Dot(cameraAxisY, reverseEyePosition);
	XMVECTOR tZ = XMVector3Dot(cameraAxisZ, reverseEyePosition);
	//一つのべクトルにまとめる
	XMVECTOR translation = XMVectorSet(tX.m128_f32[0], tY.m128_f32[1],
									   tZ.m128_f32[2], 1.0f);
	//ビュー行列に平行移動成分を設定
	matView.r[3] = translation;


#pragma region 全方向ビルボードの計算
...
#pragma region Y軸回りビルボードの計算
	...
	ybillCameraAxisZ = XMVector3Normalize(ybillCameraAxisZ);
...
void ParticleManager::Update() {
	...
	//寿命が尽きたパーティクルを削除
	particles.remove_if([](Particle& x) {return x.flame >= x.num_frame; });
	//パーティクル更新
	for (std::forward_list<Particle>::iterator it = particles.begin();
		...
		//進行度を0~1に変換
		float f = (float)it->flame / it->num_frame;
		//スケールを線形補完
